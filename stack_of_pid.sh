#########################################################################
# File Name:stack_of_pid.sh
# Author: sudoku.huang
# mail: sudoku.huang@gmail.com
# Created Time:Wed 09 Dec 2015 08:57:03 PM CST
#########################################################################
#! /bin/bash

U_ID=""
G_ID=""

function _whoami()  # get username & group
{
    local uid_gid=`id`

    gid=${uid_gid##*(}
    gid=${gid%)*}
  
    U_ID=`whoami`
    G_ID=$gid
    echo "UID:$U_ID, GID:$G_ID"
}

function get_permission() #arg like:-rwxr-xr-x
{
    local permission=$1
    local flag=$2  # 0-owner, 1-group, 2-other
    local op=$3    # r-read, w-write, x-execute

    local begin=1
    if [ $flag -eq 0 ]; then
        begin=2
    elif [ $flag -eq 1 ]; then
        begin=5
    elif [ $flag -eq 1 ]; then
        begin=8
    else
        return 0
    fi
    local end=$((begin+2))

    local check_op=""
    if [[ $op != ${op#r} ]]; then
        check_op=$check_op"r"
    fi
  
    if [[ $op != ${op#w} ]]; then
        check_op=$check_op"w"
    fi
  
    if [[ $op != ${op#x} ]]; then
        check_op=$check_op"x"
    fi
  
    local check=`echo $permission | cut -c$begin-$end`
    if [[ $check == ${check#$check_op} ]]; then
        return 0
    else
        return 1
    fi
}

function check_permission() #file
{
    local filename=$1
    local permission_to_check=$2

    if [ -z $filename ]; then
        echo "check_permission, input empty arg"
        return 0;
    fi

    if [ -z $permission_to_check ]; then
        echo "check_permission, no permission to check"
        return 0;
    fi

    if [ ! -e $filename ]; then
        echo "check_permission, ($filename) not exists!"
        return 0;
    fi

    local out=`ls -l $filename`
    local permission=`echo $out | awk '{print $1}'`
    local owner=`echo $out | awk '{print $3}'`
    local group=`echo $out | awk '{print $4}'`
    #echo "permission:$permission, owner:$owner, group:$group"

    local ret=0
    if [[ $group == $G_ID ]]; then
        if [[ $owner == $U_ID ]]; then
            ret=$(get_permission $permission 0 "r")
        else
            ret=$(get_permission $permission 1 "r")
        fi
    else
        ret=$(get_permission $permission 2 "r")
    fi
    ret=$?
    #echo "permission:$ret"
    return $ret
}

function analyse_stack()
{
    #echo "analyse_stack"
    local maps_file=$1
    local addr=`cat $1 | grep stack | awk '{print $1}'`
    let local begin=16#${addr%-*}
    let local end=16#${addr##*-}
    #echo "addr:$addr,  begin:$begin, end:$end"
    #local size=`echo "obase=16; $((16#$end - 16#$begin))" | bc`
    #let begin=16#$begin
    #let end=16#$end
    local size=$(($end - $begin))
    #echo "addr:$addr,  begin:$begin, end:$end, size:$size"
    echo $size
}

function do_stack_check()
{
    local pg_name=$0
    local pid=$1
    local stacksize_str=`ulimit -a | grep stack`
    local stacksize=`echo $stacksize_str | awk '{print $5}'`
    #stacksize='unlimited'
    echo "stacksize:"$stacksize
    stacksize=$(($stacksize * 1024))
   
    if [[ $stacksize == "unlimited" ]]; then
        echo $stacksize_str
        exit
    fi

    #ps -ef | while read line
    echo "check pid-->:$pid<---"
    #ps -ef | grep $pid | grep -v grep | grep -v $pg_name #| awk '{print $8}'
    local pname=`ps -ef | grep $pid | grep -v grep | grep -v $pg_name | grep -v bash | awk '{print $8}'`

    while true
    do
        local filename="/proc/$pid/maps"
        check_permission $filename "r"
        local permission=$?
        if [ $permission -gt 0 ]; then
            local size=$(analyse_stack $filename)
            #echo "pid:$pid, pname:$pname, size:$size, stack:$stacksize"
            printf "pid[%10d], size[%10d], limit[%10d], program[%s]\n" $pid $size $stacksize $pname
            #break;
        else
            echo "file($filename) read permission: $permission"
        fi
        sleep 1
    done
}

function main()
{
    _whoami;
    #check_permission xc_test.sh "r"
    do_stack_check $*
}

main $*


